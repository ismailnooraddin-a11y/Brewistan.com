          ...
          {!profile ? (
            <div className="panel" style={{ marginTop: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                <Link className="btn" href={`/get-started?tab=create&redirectTo=${encodeURIComponent(`/c/${cafeSlug}`)}`}>
                  Sign up <ArrowRight size={14} />
                </Link>
                <Link className="btn btn-secondary" href={`/get-started?tab=signin&redirectTo=${encodeURIComponent(`/c/${cafeSlug}`)}`}>
                  I have an account
                </Link>
              </div>
            </div>
