...
import { slugify, safeRedirect } from '@/lib/utils';
...
export function GetStartedForm() {
  ...
  const initialTab: Tab = searchParams.get('tab') === 'signin' ? 'signin' : 'create';
  ...
  // Keep tab in sync if the user navigates between ?tab=create and ?tab=signin
  useEffect(() => {
    const t = searchParams.get('tab');
    if (t === 'signin' || t === 'create') setTab(t);
  }, [searchParams]);
  ...
  async function createCafe(e: React.FormEvent<HTMLFormElement>) {
    ...
    // 1. Sign up — profile row is created automatically via DB trigger.
    //    We stash the full café details inside user_metadata so that if the
    //    user confirms their email on a different device, the dashboard can
    //    still finish setting up their café.
    ...
    // 2. If email confirmation disabled, a session exists — create cafe immediately via RPC
    if (signUpData.session) {
      ...
      if (rpcError) {
        setAlert({ kind: 'error', text: `Café setup failed: ${rpcError.message}. You can retry from the dashboard.` });
        setLoading(false);
    ...
    // 3. Email confirmation flow — stash pending cafe details, finish after callback
      ...
      <aside className="auth-side">
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ marginTop: 12, maxWidth: 360 }}>
            Create your café profile, design a card that matches your brand, launch your first campaign —
            all from one refined dashboard.
          </p>
        </div>
        <div className="panel" style={{
          ...
          <div className="icon-box" style={{ background: 'rgba(200, 155, 69, 0.16)', color: 'var(--gold-soft)' }}>
            <Coffee />
          </div>
      ...
      <section className="auth-main">
        <div className="auth-card">
          ...
          <div className="tabs" role="tablist">
            <button role="tab" aria-selected={tab === 'create'}
              className={`tab ${tab === 'create' ? 'active' : ''}`}
              onClick={() => setTab('create')}>Create café</button>
            <button role="tab" aria-selected={tab === 'signin'}
              className={`tab ${tab === 'signin' ? 'active' : ''}`}
              onClick={() => setTab('signin')}>Sign in</button>
            <button role="tab"
              className="tab"
              onClick={google}>Continue with Google</button>
          </div>
          ...
          {tab === 'create' && (
            <form onSubmit={createCafe} className="form-grid" noValidate>
              <div className="field">
                ...
                <input id="full_name" name="full_name" required placeholder="Ismail Noor" autoComplete="name" />
                {fieldErrors.full_name && <span className="error">{fieldErrors.full_name}</span>}
              </div>
              <div className="field">
                <label htmlFor="owner_phone">Owner phone</label>
                <input id="owner_phone" name="owner_phone" required placeholder="+964 750 000 0000" autoComplete="tel" />
                {fieldErrors.owner_phone && <span className="error">{fieldErrors.owner_phone}</span>}
              </div>
              <div className="field">
                <label htmlFor="email">Email address</label>
                <input id="email" name="email" required type="email" placeholder="owner@cafe.com" autoComplete="email" />
              ...
              <div className="field">
                ...
                <div style={{ position: 'relative' }}>
                  <input id="password" name="password" required minLength={8}
                    type={show ? 'text' : 'password'} placeholder="Minimum 8 characters"
              ...
              <div className="field">
                ...
                <input id="slug" name="slug" required placeholder="luna-cafe"
                  pattern="[a-z0-9][a-z0-9-]{1,38}[a-z0-9]" />
                <span className="hint">Your QR will open brewistan.com/c/<em>slug</em></span>
                {fieldErrors.slug && <span className="error">{fieldErrors.slug}</span>}
              </div>
              <div className="field">
                <label htmlFor="cafe_phone">Café phone</label>
                <input id="cafe_phone" name="cafe_phone" required placeholder="+964 750 000 0000" />
                {fieldErrors.cafe_phone && <span className="error">{fieldErrors.cafe_phone}</span>}
              </div>
              <div className="field">
          ...
          {tab === 'signin' && (
            <form onSubmit={signIn} className="form-grid" noValidate>
              ...
              <div className="field full">
                ...
                <input id="si-password" name="password" required type="password" autoComplete="current-password" />
                {fieldErrors.password && <span className="error">{fieldErrors.password}</span>}
              </div>
              <div className="full actions" style={{ marginTop: 6 }}>
                <button disabled={loading} className="btn" type="submit">
