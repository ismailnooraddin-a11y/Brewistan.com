import { readableContrast } from '@/lib/utils';

type Props = {
  cafeName: string;
  primary: string;
  secondary: string;
  watermarkOn: boolean;
  stamps: number;
  stampsRequired: number;
  rewardText: string;
  rewardsEarned?: number;
  size?: 'md' | 'lg';
  logoUrl?: string | null;
  cardText?: string | null;
  cardOpacity?: number | null;
};

export function LoyaltyCard({
  cafeName,
  primary,
  secondary,
  watermarkOn,
  stamps,
  stampsRequired,
  rewardText,
  rewardsEarned = 0,
  size = 'md',
  logoUrl,
  cardText,
  cardOpacity,
}: Props) {
  // Defensive: if owner has set an explicit text color use it, else compute contrast.
  const textColor = cardText && /^#[0-9a-fA-F]{6}$/.test(cardText)
    ? cardText
    : readableContrast(primary);

  // Defensive: campaign with 0 stamps_required would render an empty card.
  const safeRequired = Math.max(1, Math.min(50, Math.floor(stampsRequired || 0)));
  const safeStamps = Math.max(0, Math.floor(stamps || 0));
  const clamp = Math.min(safeStamps, safeRequired);

  const opacity = typeof cardOpacity === 'number' && cardOpacity >= 0.5 && cardOpacity <= 1
    ? cardOpacity
    : 1;

  const logoInitial = (cafeName?.charAt(0) || '?').toUpperCase();

  return (
    <div
      className={size === 'lg' ? 'wallet-card' : 'loyalty-card'}
      data-watermark={watermarkOn ? cafeName : ''}
      style={{
        background: `linear-gradient(135deg, ${primary}, ${secondary})`,
        color: textColor,
        opacity,
      }}
    >
      <div className="loyalty-card-top">
        <div>
          <div className="loyalty-card-sub" style={{ color: `${textColor}99` }}>Brewistan card</div>
          <h4 style={{ color: textColor }}>{cafeName}</h4>
        </div>
        <div
          className="logo-circle"
          style={{
            color: textColor,
            backgroundImage: logoUrl ? `url(${logoUrl})` : undefined,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            backgroundColor: logoUrl ? 'transparent' : undefined,
          }}
          aria-label={logoUrl ? `${cafeName} logo` : undefined}
        >
          {logoUrl ? '' : logoInitial}
        </div>
      </div>
      <div
        className="dots"
        role="progressbar"
        aria-valuenow={clamp}
        aria-valuemin={0}
        aria-valuemax={safeRequired}
        aria-label={`${clamp} of ${safeRequired} stamps`}
      >
        {Array.from({ length: safeRequired }).map((_, i) => (
          <span key={i} className={`dot ${i < clamp ? 'on' : ''}`} />
        ))}
      </div>
      <div className="loyalty-card-meta">
        <span>{clamp} / {safeRequired}</span>
        <span>{rewardsEarned > 0 ? `${rewardsEarned} reward${rewardsEarned > 1 ? 's' : ''} earned` : 'No rewards yet'}</span>
      </div>
      <div className="loyalty-card-meta" style={{ marginTop: 4 }}>
        <span>Reward</span>
        <span>{rewardText}</span>
      </div>
    </div>
  );
}
