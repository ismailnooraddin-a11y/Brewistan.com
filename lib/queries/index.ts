...
export async function getCustomerCards(customerId: string) {
  ...
  const { data } = await supabase
    ...
    .select(`
      ...
      campaign:campaigns ( id, name, stamps_required, reward_text, status, deleted_at )
    `)
...
export async function getCafeStats(cafeId: string) {
  const supabase = await createClient();
  const [customers, pending, campaigns, redemptionsAvail] = await Promise.all([
    ...
    supabase.from('campaigns').select('id', { count: 'exact', head: true }).eq('cafe_id', cafeId).eq('status', 'active').is('deleted_at', null),
    supabase.from('reward_redemptions').select('id', { count: 'exact', head: true }).eq('cafe_id', cafeId).eq('status', 'available'),
  ]);
  return {
    customers: customers.count ?? 0,
    pending: pending.count ?? 0,
    activeCampaigns: campaigns.count ?? 0,
    pendingRedemptions: redemptionsAvail.count ?? 0,
  };
}

export async function getCafeCampaigns(cafeId: string, includeDeleted = false) {
  const supabase = await createClient();
  let q = supabase
    .from('campaigns')
    .select('*')
...
/**
 * Returns the staff record + cafe info for the current user, if any.
 * Used by /staff routes to identify which café the staff member works at.
 */
export async function getStaffMembership() {
  ...
  const { data } = await supabase
    .from('staff')
    .select(`
      id, role, branch_id, is_active,
      cafe:cafes ( id, slug, cafe_name, logo_url, subscription_status, is_active )
    `)
...
export async function getCustomerActiveRedemptions() {
  const supabase = await createClient();
  const { data } = await supabase.rpc('get_my_active_redemptions');
  return data ?? [];
}

export async function getCafePendingRedemptions(cafeId: string) {
  const supabase = await createClient();
